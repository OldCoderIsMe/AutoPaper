"""Generate InfoQ-style SVG card for weekly summary."""
import os
...
def generate_infocard(
    ...
    """Generate an InfoQ-style SVG card for technical content summary.
    ...
    if style == "tech":
        primary_color = "#0066CC"  # InfoQ blue
...
4. 开发者工具链无缝集成"""
    ...
    prompt = f"""你是一个专业的技术内容设计师，擅长制作InfoQ风格的技术信息图卡片。
...
7. **右侧要点列表布局要求**（重要！）:
   ...
   - 序号单独一行，使用强调色
   - 要点内容如果超过 25 个字，必须换行显示
   - 行间距设置为 1.5-1.8 倍字号
   - 每个要点之间留出 30-35px 的垂直间距
   - 参考示例：
   ```xml
   <text x="40" y="0" font-size="18" fill="{primary_color}" font-weight="bold">1.</text>
   <text x="65" y="0" font-size="16" fill="{text_primary}">AI编程工具从对话式</text>
   <text x="65" y="22" font-size="16" fill="{text_primary}">向闭环式演进</text>
   ```
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 675" width="1200" height="675">
  ...
  <g transform="translate(60, 80)">
    ...
    <text x="30" y="50" font-family="system-ui, sans-serif" font-size="28" font-weight="bold" fill="{text_primary}">{title}</text>
    <text x="30" y="90" font-family="system-ui, sans-serif" font-size="18" fill="{text_secondary}">技术周刊 · 深度解读</text>
    <rect x="30" y="120" width="80" height="4" rx="2" fill="{primary_color}"/>
    <text x="30" y="155" font-family="system-ui, sans-serif" font-size="16" fill="{text_secondary}">Generated by AutoPaper</text>
  </g>

  <!-- 右侧区域: 核心要点 -->
  <g transform="translate(540, 80)">
    <rect x="0" y="0" width="600" height="500" rx="16" fill="{bg_card}" opacity="0.8"/>
    <text x="40" y="50" font-family="system-ui, sans-serif" font-size="24" font-weight="bold" fill="{primary_color}">本周核心看点</text>
  ...
  <text x="1140" y="640" font-family="system-ui, sans-serif" font-size="14" fill="{text_secondary}" text-anchor="end">infoq.style</text>
</svg>
```
...
"""
    ...
    try:
        ...
        raise RuntimeError(f"Failed to generate InfoQ card: {e}")


def save_infocard(svg_code: str, output_path: str):
    """Save InfoQ card to file.
...
我们看到了从简单的代码补全向自主 Agent 和自我参照迭代的跨越。
    ...
    try:
        ...
        print(f"✓ InfoQ card generated: {output_path}")
    except Exception as e:
